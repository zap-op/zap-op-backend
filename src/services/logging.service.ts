...
const sharedLoggerOpt = function (logType: string) {
	return {
		format: winston.format.combine(winston.format.timestamp(), winston.format.label({ label: logType, message: true })),
		level: "debug",
		exitOnError: false,
...
winston.loggers.add(
	INTERNAL_LOG_TYPES.MAIN_PROC,
	Object.assign({}, sharedLoggerOpt(INTERNAL_LOG_TYPES.MAIN_PROC), {
		transports: [sharedConsoleTransportOpt(), sharedFileTransportOpt(INTERNAL_LOG_TYPES.MAIN_PROC)],
	}),
);

winston.loggers.add(
	INTERNAL_LOG_TYPES.HTTP_REQUEST,
	Object.assign({}, sharedLoggerOpt(INTERNAL_LOG_TYPES.HTTP_REQUEST), {
		transports: [sharedFileTransportOpt(INTERNAL_LOG_TYPES.HTTP_REQUEST), new winston.transports.Console({ format: winston.format.cli(), level: "error" })],
	}),
);

winston.loggers.add(
	INTERNAL_LOG_TYPES.USER_SESSION,
	Object.assign({}, sharedLoggerOpt(INTERNAL_LOG_TYPES.USER_SESSION), {
		transports: [sharedFileTransportOpt(INTERNAL_LOG_TYPES.USER_SESSION), new winston.transports.Console({ format: winston.format.cli(), level: "error" })],
	}),
);
...
export function registerCustomLogger(logType: string, transportsOpt: Transport[]): winston.Logger {
	winston.loggers.add(
		logType,
		Object.assign({}, sharedLoggerOpt(logType), {
...
export function endCustomLogger(logger: winston.Logger): void {
	if (customRegisteredLogger.splice(customRegisteredLogger.indexOf(logger), 1).length > 0) logger.end();
}
